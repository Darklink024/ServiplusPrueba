...
{
    internal class BusinessMobileLogger
    {
        private static readonly ILog log = LogManager.GetLogger("MabeMobileLog");

        public BusinessMobileLogger()
        {
            ...
            roller.File = @"Logs\MabeMobileLog.txt";
        ...
        public void WriteEntry(string msg)
        {
            try
            {
                string msgEnd = "-----------------------Log Mobile Inicio------------------------";
                msgEnd += String.Format("Fecha: {0}" + Environment.NewLine, DateTime.Now.ToString());
                msgEnd += String.Format("Data: {0}" + Environment.NewLine, msg);
                msgEnd += "-----------------------Log Mobile End------------------------";
                log.Info(msgEnd);
            }
            catch (Exception ex)
            {
